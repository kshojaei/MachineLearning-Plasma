...
nblo = np.array([0.15054377, 0.45485725, 0.3301524,  0.64300798, 1.54673555, 5.31718698, 3.12825295])
nbho = np.array([0.25990531, 0.55249339, 0.66004653, 0.67887192, 1.77270402, 5.93418609, 3.27272377])
...
bmclo = np.array([0.12083995, 0.44978812, 0.32003106, 0.65003207, 1.58922045, 5.38021566, 3.09015034])
bmcho = np.array([0.23369988, 0.53193677, 0.57840714, 0.68543723, 1.76852655, 6.11697461, 3.34159832])
...
bvlo = np.array([0.11855429, 0.44143269, 0.29583846, 0.63291286, 1.52103693, 5.20614821, 3.08891363])
bvho = np.array([0.24058822, 0.54749943, 0.66303919, 0.67546961, 1.75915471, 5.8909922, 3.18951483])
...
nbl = np.array([0.24915901, 0.5469997,  0.77163387, 0.62591783, 1.77825036, 5.79535114, 3.07767839])
nbh = np.array([0.35207807, 0.63373568, 1.1281217,  0.66022686, 1.99079446, 6.3966354, 3.21816306])
# bayesian MC
bmcm = np.array([0.2960552,  0.56797192, 0.91116277, 0.69511618, 1.83497538, 5.87874525, 3.17657526])
bmcl = np.array([0.18332417, 0.47567198, 0.60767441, 0.65477075, 1.63629825, 5.20435807, 2.98831272])
bmch = np.array([0.38811567, 0.64378626, 1.23113154, 0.73237779, 2.03095557, 6.47448422, 3.37609256])
# bayesian Vi
bvm = np.array([0.33547666, 0.59606597, 1.09475095, 0.71407412, 1.99346334, 6.2151235, 3.08538195])
bvl = np.array([0.26621647, 0.54554347, 0.83846774, 0.64123814, 1.39969651, 4.86625228, 2.80213431])
bvh = np.array([0.40112246, 0.64671743, 1.35114493, 0.78682065, 2.52081983, 7.44820292, 3.34462983])
'''
...
plt.plot(E, savt(pred(E,tt[0],tt[1],tt[2],tt[3],tt[4],tt[5],)), 'lime', linewidth=3)

#plt.plot(E, sav(pred(E,nbl[0],nbm[1],nbm[2],nbm[3],nbh[4],nbm[5])), 'b', linewidth=2)
#plt.plot(E, sav(pred(E,nbm[0],nbl[1],nbl[2],nbm[3],nbl[4],nbm[5])), 'b', linewidth=2)

plt.fill_between(E,sav(pred(E,nbl[0],nbm[1],nbm[2],nbm[3],nbh[4],nbm[5])),
                 sav(pred(E,nbm[0],nbl[1],nbl[2],nbm[3],nbl[4],nbm[5])), alpha= 1
                 )


#plt.plot(E, sav(pred(E,nblo[0],nbm[1],nbm[2],nbm[3],nbho[4],nbm[5])), 'k', linewidth=2)
#plt.plot(E, sav(pred(E,nbm[0],nblo[1],nblo[2],nbm[3],nblo[4],nbm[5])), 'k', linewidth=2)


plt.fill_between(E,sav(pred(E,nblo[0],nbm[1],nbm[2],nbm[3],nbho[4],nbm[5])),
                 sav(pred(E,nbm[0],nblo[1],nblo[2],nbm[3],nblo[4],nbm[5])), color='c', alpha=0.2)

plt.legend(['Predicted $f_p$','Measured $f_p$', '68% Confidence Interval', '95% Confidence Interval'])

#plt.plot(E, sav(pred(E,nbm[0],nbm[1],nbm[2],nbm[3],nbm[4],nbm[5],)), 'r', linewidth=2)
...
#plt.plot(E, sav(pred(E,bvl[0],bvm[1],bvm[2],bvm[3],bvh[4],bvm[5])), 'b', linewidth=2)
#plt.plot(E, sav(pred(E,bvm[0],bvl[1],bvl[2],bvm[3],bvl[4],bvm[5])), 'b', linewidth=2)

plt.fill_between(E,sav(pred(E,bvl[0],bvm[1],bvm[2],bvm[3],bvh[4],bvm[5])),
                 sav(pred(E,bvm[0],bvl[1],bvl[2],bvm[3],bvl[4],bvm[5])), alpha=1)

#plt.plot(E, sav(pred(E,bvlo[0],bvm[1],bvm[2],bvm[3],bvho[4],bvm[5])), 'g', linewidth=2)
#plt.plot(E, sav(pred(E,bvm[0],bvlo[1],bvlo[2],bvm[3],bvlo[4],bvm[5])), 'g', linewidth=2)

plt.fill_between(E, sav(pred(E,bvlo[0],bvm[1],bvm[2],bvm[3],bvho[4],bvm[5])),
                 sav(pred(E,bvm[0],bvlo[1],bvlo[2],bvm[3],bvlo[4],bvm[5])), color= 'c', alpha=0.2)
...
#plt.plot(E, sav(pred(E,bmcl[0],bmcm[1],bmcm[2],bmcl[3],bmcm[4],bmcl[5])), 'b', linewidth=2)
#plt.plot(E, sav(pred(E,bmcm[0],bmcl[1],bmcl[2],bmcm[3],bmcm[4],bmch[5])), 'b', linewidth=2)

plt.fill_between(E,sav(pred(E,bmcl[0],bmcm[1],bmcm[2],bmcl[3],bmcm[4],bmcl[5])),
                 sav(pred(E,bmcm[0],bmcl[1],bmcl[2],bmcm[3],bmcm[4],bmch[5])), alpha=1)

#plt.plot(E, sav(pred(E,bmclo[0],bmcm[1],bmcm[2],bmclo[3],bmcm[4],bmclo[5])), 'g', linewidth=2)
#plt.plot(E, sav(pred(E,bmcm[0],bmclo[1],bmclo[2],bmcm[3],bmcm[4],bmcho[5])), 'g', linewidth=2)

plt.fill_between(E, sav(pred(E,bmclo[0],bmcm[1],bmcm[2],bmclo[3],bmcm[4],bmclo[5])),
                 sav(pred(E,bmcm[0],bmclo[1],bmclo[2],bmcm[3],bmcm[4],bmcho[5])),alpha=0.2, color='c')
...
fig.savefig('EEPFS_sim_nobayes_fico.png', format='png', dpi=600)
