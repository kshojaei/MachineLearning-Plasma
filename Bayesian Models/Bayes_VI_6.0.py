...
kernel_divergence_fn=lambda q, p, _: tfp.distributions.kl_divergence(q, p) / (x_train.shape[0]/b_size)
bias_divergence_fn=lambda q, p, _: tfp.distributions.kl_divergence(q, p) /(x_train.shape[0]/b_size)

inputs = Input(shape=(19,))

hidden = tfp.layers.DenseFlipout(60,bias_posterior_fn=tfp.layers.util.default_mean_field_normal_fn(),
...
hidden = tfp.layers.DenseFlipout(30,bias_posterior_fn=tfp.layers.util.default_mean_field_normal_fn(),
                           bias_prior_fn=tfp.layers.default_multivariate_normal_fn,
                           kernel_divergence_fn=kernel_divergence_fn,
                           bias_divergence_fn=bias_divergence_fn,activation="tanh")(hidden)
...
params = tfp.layers.DenseFlipout(14,bias_posterior_fn=tfp.layers.util.default_mean_field_normal_fn(),
                           bias_prior_fn=tfp.layers.default_multivariate_normal_fn,
                           kernel_divergence_fn=kernel_divergence_fn,
...
model.compile(Adam(learning_rate=4e-4), loss= nll, metrics= ['mse'])
...
model.save("bayes_VI_6.h5")
model.save_weights('bayes_VI_weights_6.h5')
# load model

'''
model.load_weights('bayes_VI_weights_6.h5')
#model.summary()
# summarize model
#model.summary()
# evaluate the model
...
plt.title("Bayesian Model: Variational Inference", fontsize=16, fontweight=16)
...
'''
np.savetxt('Total_LM_scatter_VIbayes_500.csv',  
